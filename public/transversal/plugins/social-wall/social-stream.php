...
class SocialStream {
    ...
    function init( $attr, $echo = true, $args = null, $ajax_feed = array(), $loadmore = array() ) {
        ...
        $SB_PATH = ! empty($GLOBALS['SB_PATH']) ? $GLOBALS['SB_PATH'] : SB_PATH;
                ...
                foreach ($breakpoints as $bpkey => $breakpoint) {
                    ...
                    if (@$attr['fixWidth'] == 'false') {
                        if ($is_grid) {
                            ...
                            $bpcol[$bpkey] .= "$sbcolumn2 { width: $bpgrid%; margin-bottom: {$bpyut}px; }";
                            $bpcol[$bpkey] .= "
...
@media (max-width: 319px) { $bpcol[6] }";
                    ...
                    if ( ! empty($feed) ) {
	                    ...
	                    foreach ($feed as $data) {
	                        ...
	                        <span class="sb-share sb-tweet">
	                            ...
	                            <a href="https://twitter.com/intent/retweet?tweet_id=' . @$data->id_str . '&via=' . $data->user->screen_name . '"'.$target.'><i class="sb-bico sb-retweet">Retweet</i> ' . $data->retweet_count . '</a>
        ...
        if (@$attr['iframe'] == 'slide') {
            $iframe_output = $iframe_slideshow = '
				...
				$(".sboard .sb-thumb .icbox").colorbox({
                    photo: true,
                    href: function() {
        ...
        switch ( $feed_key ) {
            case 'facebook':
                ...
                if ( ! empty($GLOBALS['api']['facebook']['facebook_accounts']) ) {
                    ...
                    $resetPages = null;//reset($resetAccounts['pages']);
                    ...
                    if ($i == 2 || $i == 3 || $i == 1) {
                        ...
                        if ($get_feed) {
                            ...
                            try {
                                ...
                                $feed = new stdClass();
                                if ($i == 2) {
                                    //$content = $medias = $instagram->getMediasByTag($feed_value, $results, $next_max_id);
                                    ...
                                    if ( ! is_numeric($feed_value) ) {
										...
										if ( (isset($user_info['meta']) && isset($user_info['meta']['code']) && $user_info['meta']['code'] !== 200) || empty($user_info['data']) ) {
											sb_log( 'Instagram error: Cant load user data for ' . $feed_value . '. Please, try again later.' );
										} else {
