    ...
    function create_item( $feed_class, $param, $attr = array(), $output = array(), $sbi = 0, $i = 0, $ifeed = 0 ) {
        ...
        if ( $attr['type'] == 'timeline' ) {
          ...
          <div class="timeline-row"'.$idstr.'>
            ...
            $icon = ( @$param['icon'][0] ) ? '<img class="sb-img" src="'.$param['icon'][0].'" style="vertical-align:middle" alt="' . $feed_class . '">' : '<i class="sb-icon sb-' . $feed_class . '"></i>';
            $tag = ( $attr['type'] != 'feed' || @$attr['carousel'] ) ? 'div' : 'li';
            
            $out1 = '
            <'.$tag.' class="sb-item sb-' . $feed_class . $filtername . $sbinline.'"'.$inline.'>
                ' . $inner;
