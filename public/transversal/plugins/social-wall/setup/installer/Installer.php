...
class Installer {
    ...
    public function generalAction() {
        ...
        foreach ($locale as $locKey => $locVal) {
    ...
    public function apiAction() {
        ...
        if ( @$_REQUEST['api'] == 'instagram' ) {
            if ( ! empty( $_REQUEST['access_token'] ) ) {
                ...
                $feed_url = 'https://api.instagram.com/v1/users/self/?access_token=' . $access_token;
                if ( $userDataString = $this->apiGet($feed_url) ) {
                    ...
                    if ( ! empty($userData->data) ) {
                        ...
                        $newAccount = array($userData->data->id => $userAccounts);
        ...
        if ( ! empty($instagram_accounts) ) {
            foreach ($instagram_accounts as $iaccount) {
                $iaccounts .= '
                <div class="accounts-wrapper">
                    <img src="' . @$iaccount['profile_picture'] . '" alt="">
                    <p><u>ID:</u> ' . $iaccount['id'] . '</p>
                    <p><u>Username:</u> ' . @$iaccount['username'] . '</p>
                    <p><u>Full Name:</u> ' . @$iaccount['full_name'] . '</p>
        ...
        $this->view->vars = array(
            ...
            'instagram_token_url' => TOKEN_SERVER.'instagram-access-token/?return_uri='.$encoded_base_url,
            'linkedin_token_url' => TOKEN_SERVER.'linkedin-access-token/?return_url='.$encoded_base_url
        );

        foreach ( array('facebook', 'twitter', 'linkedin') as $network ) {
